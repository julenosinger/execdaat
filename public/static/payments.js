...
async function executePayment() {
  ...
  try {
    ...
    // Do NOT auto-download — store receipt, show View Receipt button only
    payState.receipt = receiptData;
...
function payOpenReceiptModal(receiptData) {
  ...
  if (!modal || !body || !r) { showToast('No receipt available', 'error'); return; }
...
function buildReceiptHTML(r) {
  ...
  return `
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;">
      <button onclick="generatePayReceiptPDF(payState.receipt || (payState.history.find(h=>h.id==='${r.id}')||payState.scheduled.find(s=>s.id==='${r.id}')), false)"
        style="flex:1;min-width:120px;padding:9px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.25);border-radius:10px;color:#f87171;font-size:12px;font-weight:700;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;gap:6px;"
        onmouseover="this.style.background='rgba(239,68,68,0.15)'" onmouseout="this.style.background='rgba(239,68,68,0.08)'">
        <i class="fas fa-file-pdf"></i> Download PDF
      </button>
      <button onclick="downloadPayReceiptById('${r.id}')"
...
function downloadPayReceiptById(id) {
  ...
  const fname = r.txHash ? 'arc-receipt-' + r.txHash.slice(0,10) + '.json'
                         : 'arc-sched-' + (r.id || Date.now()) + '.json';
...
function generatePayReceiptPDF(r, autoDownload) {
  ...
  if (jsPDF) {
    try {
      ...
      const fname = r.txHash ? 'arc-receipt-' + r.txHash.slice(0, 10) + '.pdf' : 'arc-receipt-' + Date.now() + '.pdf';
      doc.save(fname);
...
// ─── JSON download (legacy) ────────────────────────────────────────────────────
function downloadPayReceipt(format) {
  const r = payState.receipt;
  if (!r) { showToast('No receipt available', 'error'); return; }
  if (format === 'json') downloadPayReceiptById(r.id);
  else if (format === 'pdf') generatePayReceiptPDF(r, false);
}
...
window.downloadPayReceiptById  = downloadPayReceiptById;
