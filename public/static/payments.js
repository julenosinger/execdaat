...
// ═══════════════════════════════════════════════════════════════════════════════
// PAY V2 UI — New reference design wiring
// ═══════════════════════════════════════════════════════════════════════════════
function payV2TimeAgo(ts) {
...
function payV2SyncSender() {
  const addr = window.walletState && window.walletState.address;
  const el = document.getElementById('pay-wallet-short-display');
  if (el) el.value = addr ? addr.slice(0, 8) + '\u2026' + addr.slice(-6) : '';
  const compat = document.getElementById('pay-wallet-short');
  if (compat) compat.textContent = addr || '';
  payV2UpdateAvail();
  payV2UpdateHeaderBalances();
}

function payV2UpdateAvail() {
  const avail = document.getElementById('pay-avail-display');
  if (!avail) return;
  const balUsdc = (document.getElementById('pay-balance-usdc') || {}).textContent || '--';
  const balEurc = (document.getElementById('pay-balance-eurc') || {}).textContent || '--';
  const lbl = document.getElementById('pay-amount-token-label');
  const token = lbl ? lbl.textContent : 'USDC';
  avail.textContent = (token === 'USDC' ? balUsdc : balEurc) + ' ' + token;
}
window.payUpdateAvail = payV2UpdateAvail;

function payV2UpdateHeaderBalances() {
  var usdcRaw = (document.getElementById('pay-balance-usdc') || {}).textContent || '--';
  var eurcRaw = (document.getElementById('pay-balance-eurc') || {}).textContent || '--';
  function stripNum(s) {
    var n = parseFloat(s.replace(/[^\d.]/g, ''));
    return isNaN(n) ? '--' : n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  }
  var hu = document.getElementById('pay-header-usdc');
  var he = document.getElementById('pay-header-eurc');
  if (hu) hu.textContent = stripNum(usdcRaw);
  if (he) he.textContent = stripNum(eurcRaw);
}
window.payUpdateHeaderBalances = payV2UpdateHeaderBalances;

window.payUpdateUsdEquiv = function() {
  var amt = parseFloat((document.getElementById('pay-amount') || {}).value) || 0;
  var el = document.getElementById('pay-usd-equiv');
  if (el) el.textContent = amt.toFixed(2);
  var fee = 0.12;
  var feeEl = document.getElementById('pay-fee-display');
  var totEl = document.getElementById('pay-total-display');
  if (feeEl) feeEl.textContent = fee.toFixed(2);
  if (totEl) totEl.textContent = (amt + fee).toFixed(2);
};

window.payCheckAddress = function(val) {
  var badge = document.getElementById('pay-addr-valid');
  if (!badge) return;
  var valid = /^0x[0-9a-fA-F]{40}$/.test((val || '').trim());
  if (valid) badge.classList.add('show'); else badge.classList.remove('show');
};

window.payPasteAddress = function() {
  navigator.clipboard && navigator.clipboard.readText().then(function(txt) {
    var el = document.getElementById('pay-recipient');
    if (el) { el.value = txt.trim(); el.dispatchEvent(new Event('input')); }
  }).catch(function() {});
};

function payV2RenderHistory() {
  var container = document.getElementById('pay-history-list');
  var empty     = document.getElementById('pay-hist-empty');
  if (!container) return;
  var history = [];
  try { history = JSON.parse(localStorage.getItem('arc_pay_history') || '[]'); } catch(e) {}
  container.querySelectorAll('.pay-hist-item').forEach(function(el) { el.remove(); });
  if (!history.length) {
    if (empty) empty.style.display = '';
    return;
  }
  if (empty) empty.style.display = 'none';
  history.slice(0, 6).forEach(function(tx) {
    var isOut  = tx.type !== 'received';
    var item   = document.createElement('div');
    item.className = 'pay-hist-item';
    var amtStr = (isOut ? '- ' : '+ ') + parseFloat(tx.amount || 0).toFixed(2) + ' ' + (tx.token || 'USDC');
    var sub    = isOut
      ? 'To: '   + ((tx.recipient || tx.to   || '').slice(0, 14) + '\u2026')
      : 'From: ' + ((tx.sender   || tx.from  || 'Liquidity Pool').slice(0, 14) + '\u2026');
    var age  = tx.timestamp ? payV2TimeAgo(tx.timestamp) : '';
    var hash = tx.txHash || tx.hash || '';
    var arrowCls  = isOut ? 'pay-hist-arrow-out' : 'pay-hist-arrow-in';
    var arrowIcon = isOut ? 'fa-arrow-up-right'  : 'fa-arrow-down-left';
    var amtCls    = isOut ? 'text-red-400'       : 'text-green-400';
    var status    = (tx.status || 'success').toUpperCase();
    var d = document.createElement('div');
    d.className = 'pay-hist-arrow ' + arrowCls;
    d.innerHTML = '<i class="fas ' + arrowIcon + '"></i>';
    item.appendChild(d);
    var info = document.createElement('div');
    info.className = 'flex-1 min-w-0';
    var amtDiv = document.createElement('div');
    amtDiv.className = 'pay-hist-amount ' + amtCls;
    amtDiv.textContent = amtStr;
    var subDiv = document.createElement('div');
    subDiv.className = 'pay-hist-sub truncate';
    subDiv.textContent = sub;
    info.appendChild(amtDiv);
    info.appendChild(subDiv);
    item.appendChild(info);
    var meta = document.createElement('div');
    meta.className = 'pay-hist-meta';
    var badge = document.createElement('div');
    badge.className = 'pay-status-success pay-status-badge';
    badge.textContent = status;
    var timeDiv = document.createElement('div');
    timeDiv.className = 'pay-hist-time';
    timeDiv.textContent = age;
    meta.appendChild(badge);
    meta.appendChild(timeDiv);
    item.appendChild(meta);
    if (hash) item.onclick = function() { window.open('https://testnet.arcscan.app/tx/' + hash, '_blank'); };
    container.appendChild(item);
  });
}

function payV2RenderQueue() {
  var container = document.getElementById('pay-queue-items');
  var empty     = document.getElementById('pay-queue-empty');
  var badge     = document.getElementById('pay-queue-count');
  if (!container) return;
  var list = [];
  try { list = JSON.parse(localStorage.getItem('arc_agent_queue') || '[]'); } catch(e) {}
  container.querySelectorAll('.pay-queue-item').forEach(function(el) { el.remove(); });
  if (!list.length) {
    if (empty) empty.style.display = '';
  ...
  if (empty) empty.style.display = 'none';
  var active = list.filter(function(q) { return q.status === 'active'; }).length;
  if (badge) badge.textContent = active + ' ACTIVE';
  list.slice(0, 4).forEach(function(q, i) {
    var item = document.createElement('div');
    item.className = 'pay-queue-item';
    var iconCls = i % 2 === 0 ? 'pay-queue-icon-purple' : 'pay-queue-icon-orange';
    var iconFa  = i % 2 === 0 ? 'fa-layer-group'         : 'fa-sync-alt';
    var statCls = q.status === 'active' ? 'pay-status-active' : 'pay-status-queued';
    var statLbl = (q.status || 'QUEUED').toUpperCase();
    var icon = document.createElement('div');
    icon.className = 'pay-queue-icon ' + iconCls;
    icon.innerHTML = '<i class="fas ' + iconFa + '"></i>';
    var info = document.createElement('div');
    info.className = 'flex-1 min-w-0';
    var nameDiv = document.createElement('div');
    nameDiv.className = 'pay-queue-name truncate';
    nameDiv.textContent = q.name || ('Agent Task #' + (i + 1));
    var typeDiv = document.createElement('div');
    typeDiv.className = 'pay-queue-type';
    typeDiv.textContent = q.type || 'AUTOMATED EXEC';
    info.appendChild(nameDiv);
    info.appendChild(typeDiv);
    var stat = document.createElement('span');
    stat.className = 'pay-status-badge ' + statCls;
    stat.textContent = statLbl;
...
// Override token select to also update UI labels
var _payOrigSelectToken = window.selectPayToken;
window.selectPayToken = function(token) {
  if (_payOrigSelectToken) _payOrigSelectToken(token);
  var usdc = document.getElementById('pay-token-usdc');
  var eurc = document.getElementById('pay-token-eurc');
  if (usdc) usdc.classList.toggle('active', token === 'USDC');
  if (eurc) eurc.classList.toggle('active', token === 'EURC');
  var lbl = document.getElementById('pay-amount-token-label');
  if (lbl) lbl.textContent = token;
  payV2UpdateAvail();
};

// Override validatePayForm to toggle .ready on button
var _payOrigValidate = window.validatePayForm;
window.validatePayForm = function() {
  if (_payOrigValidate) _payOrigValidate();
  var btn  = document.getElementById('pay-send-btn');
  if (!btn) return;
  var addr = ((document.getElementById('pay-recipient') || {}).value || '').trim();
  var amt  = parseFloat((document.getElementById('pay-amount') || {}).value) || 0;
  var ok   = /^0x[0-9a-fA-F]{40}$/.test(addr) && amt > 0 && window.walletState && window.walletState.address;
  if (ok) { btn.classList.add('ready'); btn.disabled = false; }
  else    { btn.classList.remove('ready'); btn.disabled = true; }
};

// Override renderPaymentHistory to also update new cards
var _payOrigRender = window.renderPaymentHistory;
window.renderPaymentHistory = function() {
  if (_payOrigRender) _payOrigRender();
  payV2RenderHistory();
};

// Override loadPayments to also update queue card
var _payOrigLoad = window.loadPayments;
window.loadPayments = function() {
  if (_payOrigLoad) _payOrigLoad();
  payV2RenderQueue();
};

// Override refreshPaymentBalances to also update header cards
var _payOrigRefresh = window.refreshPaymentBalances;
window.refreshPaymentBalances = async function() {
  if (_payOrigRefresh) await _payOrigRefresh();
  payV2UpdateHeaderBalances();
  payV2UpdateAvail();
  payV2SyncSender();
};

// Main init called from DOMContentLoaded
window.payV2Init = function() {
  payV2SyncSender();
  payV2RenderHistory();
  payV2RenderQueue();

  // MutationObserver on hidden balance spans
  var usdcSpan = document.getElementById('pay-balance-usdc');
  var eurcSpan = document.getElementById('pay-balance-eurc');
  if (usdcSpan || eurcSpan) {
    var obs = new MutationObserver(function() {
      payV2UpdateHeaderBalances();
      payV2UpdateAvail();
    });
    var cfg = { childList: true, subtree: true, characterData: true };
    if (usdcSpan) obs.observe(usdcSpan, cfg);
    if (eurcSpan) obs.observe(eurcSpan, cfg);
  }
  ...
  window.addEventListener('walletDisconnected', function() { payV2SyncSender(); });

  // Override switchTab to refresh on payments open
  ...
  document.dispatchEvent(new Event('payV2Ready'));
};

// Auto-init when DOM ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() { window.payV2Init(); });
} else {
  setTimeout(function() { window.payV2Init(); }, 50);
}
